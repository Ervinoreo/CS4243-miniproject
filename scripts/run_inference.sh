...
#SBATCH --output=./logs/infer_output_%j.slurmlog
#SBATCH --error=./logs/infer_error_%j.slurmlog

# Check if both arguments are provided
if [ "$#" -ne 2 ]; then
    echo "Usage: sbatch $0 <model_path> <input_path>"
    exit 1
fi
...
echo "Running script: $INPUT_PATH"
...
python "$INPUT_PATH" --model_folder "$MODEL_PATH" --test_folder data/labeled_test_resized_28x28
