...
def wavelet_features(img, wavelet, levels=max_level):
    """Extract comprehensive wavelet features from an image including raw coefficients and statistics"""
    ...
    for lvl, coeff in enumerate(coeffs):
        if lvl == 0:
            ...
            LL = coeff
            # Add flattened raw coefficients
            raw_coeffs = LL.flatten()
            for i, val in enumerate(raw_coeffs):
                features[f"{wavelet}_L{lvl}_LL_raw_{i}"] = val
            ...
            for name, band in subbands.items():
                # Add flattened raw coefficients
                raw_coeffs = band.flatten()
                for i, val in enumerate(raw_coeffs):
                    features[f"{wavelet}_L{lvl}_{name}_raw_{i}"] = val
                # Add statistical features
...
def extract_all_wavelet_features(img_path):
    """Extract all wavelet features from a single image including raw coefficients and statistics"""
...
def main():
    parser = argparse.ArgumentParser(description="Train MLP classifier on wavelet features (raw coefficients + statistics)")
    parser.add_argument("input_folder", type=str, help="Path to input folder with class subfolders")
    parser.add_argument("--epochs", type=int, default=100, help="Number of training epochs")
    parser.add_argument("--hidden_sizes", nargs='+', type=int, default=[1024, 512, 256], 
                       help="Hidden layer sizes (e.g., --hidden_sizes 1024 512 256)")
    ...
    print(f"Feature dimension: {features_scaled.shape[1]} (includes raw wavelet coefficients + statistics)")
    print(f"Number of classes: {len(class_names)}")
    print(f"Wavelets used: {wavelets}")
    print(f"Wavelet levels: {max_level}")
    ...
    config = {
        ...
        'feature_composition': 'raw_wavelet_coefficients_and_statistics',
        'wavelets_used': wavelets,
        'wavelet_levels': max_level,
