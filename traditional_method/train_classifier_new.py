...
def calculate_energy(image):
    """Calculate energy of an image (sum of squares)."""
    return np.sum(image.astype(np.float32) ** 2)


def calculate_entropy(image):
    """Calculate entropy of an image based on histogram."""
    # Calculate histogram
...
def calculate_power(image):
    """Calculate power of an image (average energy)."""
    return np.mean(image.astype(np.float32) ** 2)
    ...
    try:
            ...
            try:
                ...
                for feature_name, feature_img in spatial_features.items():
                    features.extend([
                        np.mean(feature_img),
                        np.var(feature_img),
                        calculate_energy(feature_img),
                        calculate_entropy(feature_img),
                        calculate_power(feature_img)
                    ])
        ...
        try:
            features.extend([
                np.mean(image),
                np.var(image),
                calculate_energy(image),
                calculate_entropy(image),
                calculate_power(image),
                np.sum(image > 127),
                np.sum(image < 64),
    ...
    def extract_features(self, image):
        ...
        if self.use_spatial:
            try:
                ...
                for feature_name, feature_img in spatial_features.items():
                    # Extract statistical features from each filtered image (mean, variance, energy, entropy, power)
                    features.extend([
                        np.mean(feature_img),
                        np.var(feature_img),
                        calculate_energy(feature_img),
                ...
                features.extend([0] * (7 * 5))  # 7 filters * 5 statistics each (mean, variance, energy, entropy, power)
        ...
        if self.use_freq:
            try:
                ...
                for feature_name, feature_img in freq_features.items():
                    # Extract statistical features from each transformed image (mean, variance, energy, entropy, power)
                    features.extend([
                        np.mean(feature_img),
                        np.var(feature_img),
                        calculate_energy(feature_img),
                ...
                features.extend([0] * (6 * 5))  # 6 transforms * 5 statistics each (mean, variance, energy, entropy, power)
        ...
        if self.use_texture:
            try:
                ...
                for feature_name, feature_img in texture_features.items():
                    # Extract statistical features from each texture analysis (mean, variance, energy, entropy, power)
                    features.extend([
                        np.mean(feature_img),
                        np.var(feature_img),
                        calculate_energy(feature_img),
                ...
                features.extend([0] * (6 * 5))  # 6 texture methods * 5 statistics each (mean, variance, energy, entropy, power)
