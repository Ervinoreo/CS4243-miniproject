...
from traditional_method.train_multilayer_classifier import FeatureExtractor, CharacterMLP
...
class CharacterClassifier:
    ...
    def __init__(self, model_path, config_path):
        ...
        # Get layer configurations from the config file
        layer_configs = self.config.get("layer_configs", None)
        
        # If no layer configs found, check for old format and create default
        if layer_configs is None:
            print("No layer_configs found in config, using default 3-layer configuration")
            layer_configs = None  # Will use default in FeatureExtractor
        else:
            print(f"Using saved layer configuration: {len(layer_configs)} layers")

        # Initialize FeatureExtractor with the layer configuration
        self.feature_extractor = FeatureExtractor(layer_configs=layer_configs)
        
        # Print layer configuration for debugging
        if hasattr(self.feature_extractor, 'layer_configs'):
            print("Feature extraction layers:")
            for i, layer in enumerate(self.feature_extractor.layer_configs):
