#!/usr/bin/env python3
"""
Character-Level ResNet50 Model for CAPTCHA Recognition
This script implements a ResNet50 model to classify individual characters, then aggregates them for CAPTCHA-level evaluation.
"""
...
class CharacterDataProcessor:
    ...
    def preprocess_image(self, image_path):
        """Preprocess a single character image for ResNet50"""
        # Read image
...
class CharacterResNet50(nn.Module):
    ...
    def __init__(self, num_classes=36, pretrained=True):
        ...
        if self.freeze_early_layers:
            # Freeze conv1, bn1, and first two residual blocks
...
class CharacterTrainer:
    """Training class for Character ResNet50"""
    ...
    def __init__(self, model, device='cpu'):
        ...
        for name, param in model.named_parameters():
            if 'fc' in name:  # New classifier layers
        ...
        self.optimizer = optim.Adam([
            {'params': pretrained_params, 'lr': 0.0001},  # Lower lr for pretrained
    ...
    def train(self, train_loader, val_loader, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter = 0
                torch.save(self.model.state_dict(), 'best_character_resnet50_model.pth')
...
def main():
    ...
    BATCH_SIZE = 32  # Reduced batch size for ResNet50
    EPOCHS = 50  # Fewer epochs for pretrained model
    
    # Check for GPU
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    print("=== Character-Level ResNet50 Model (PyTorch) ===")
    ...
    # Build model
    print("\nBuilding model...")
    model = CharacterResNet50(num_classes=processor.num_classes, pretrained=True)
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_character_resnet50_model.pth'))
