#!/usr/bin/env python3
"""
Baseline ResNet Model for CAPTCHA Recognition
This script implements a ResNet baseline model to recognize text in CAPTCHA images using PyTorch.
"""
...
class CaptchaDataProcessor:
    ...
    def preprocess_image(self, image_path):
        """Preprocess a single image for ResNet"""
        # Read image
        ...
        # Apply ImageNet-style normalization for better pretrained model performance
        # Using grayscale ImageNet mean and std (approximated)
        mean = 0.485  # Approximate grayscale mean
        std = 0.229   # Approximate grayscale std
...
class CaptchaBaseline(nn.Module):
    """Baseline ResNet-50 + RNN Model for CAPTCHA Recognition using CTC Loss"""
    
    def __init__(self, img_width=200, img_height=80, num_classes=37):
        super(CaptchaBaseline, self).__init__()
        self.img_width = img_width
        self.img_height = img_height
        self.num_classes = num_classes  # 36 characters + 1 blank token
        
        # Load pretrained ResNet-50 and modify for grayscale input
        print("Loading pretrained ResNet-50...")
        resnet = models.resnet50(pretrained=True)
        
        # Modify first conv layer for grayscale input (1 channel instead of 3)
        # This allows us to use pretrained weights while adapting to grayscale images
        self.resnet_conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        
        # Initialize weights from pretrained RGB conv1 by averaging across channels
        # This preserves learned features while adapting to single-channel input
        with torch.no_grad():
            self.resnet_conv1.weight = nn.Parameter(resnet.conv1.weight.mean(dim=1, keepdim=True))
        
        # Use ResNet layers except the final classification layers
        self.resnet_bn1 = resnet.bn1
        self.resnet_relu = resnet.relu
        # Skip maxpool to reduce downsampling from 32x to 16x (to match baseline CNN)
        # self.resnet_maxpool = resnet.maxpool  # Commented out
    ...
    def forward(self, x):
        ...
        # ResNet-50 Feature Extraction (modified to match baseline CNN timesteps)
        x = self.resnet_conv1(x)
        x = self.resnet_bn1(x)
        x = self.resnet_relu(x)
        # Skip maxpool to get 16x downsampling instead of 32x (matches baseline CNN)
        # x = self.resnet_maxpool(x)  # Commented out
        ...
        # x shape: (batch, channels=2048, height, width)
...
class CaptchaTrainer:
    """Training class for CAPTCHA Baseline with CTC Loss"""
    ...
    def __init__(self, model, device='cpu'):
        ...
        # Use different learning rates for pretrained ResNet and new layers
        resnet_params = []
        new_params = []
        
        # Separate ResNet parameters from new RNN/output parameters
        ...
        # Lower learning rate for pretrained ResNet, higher for new layers
        self.optimizer = optim.Adam([
            {'params': resnet_params, 'lr': 1e-4},  # Lower LR for pretrained layers
    ...
    def train(self, train_loader, val_loader, epochs=30):
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(self.model.state_dict(), 'best_captcha_resnet50_model.pth')
...
def main():
    ...
    TRAIN_DIR = "./train"
    TEST_DIR = "./test"
    BATCH_SIZE = 16  # Reduced for ResNet-50 (larger model needs more memory)
    EPOCHS = 200
    ...
    print("=== CAPTCHA ResNet Baseline Model (PyTorch) ===")
    ...
    # Build model
    print("\nBuilding model...")
    model = CaptchaBaseline(num_classes=processor.num_classes)
    
    # Count parameters
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    # Count ResNet vs new parameters
    resnet_params = sum(p.numel() for name, p in model.named_parameters() if 'resnet_' in name)
    new_params = sum(p.numel() for name, p in model.named_parameters() if 'resnet_' not in name)
    
    print(f"Total parameters: {total_params:,}")
    print(f"Trainable parameters: {trainable_params:,}")
    print(f"ResNet-50 parameters: {resnet_params:,}")
    print(f"New RNN/Output parameters: {new_params:,}")
    print(f"Feature map dimensions: {model.feature_height} x {model.feature_width}")
    print(f"RNN timesteps: {model.feature_width}")
    ...
    model.load_state_dict(torch.load('best_captcha_resnet50_model.pth'))
